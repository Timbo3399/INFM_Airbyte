...
| File-Server | `hso_fileserver` | `nginx:alpine` | liefert CSV-Flatfiles, kopiert sie nach `oss_local_root` | `8888` → 80 |
...
- **File-Connector via `local` Storage Provider** (`/local/<datei>.csv`) statt HTTP: Der `source-file`-Connector erzwingt bei „HTTPS: Public Web" eine TLS-Verbindung. Der File-Server kopiert die CSVs beim Start in das Volume `oss_local_root`, das Airbyte als `/local/` einbindet.
- **MySQL mit `--local-infile=1`:** vom Airbyte-MySQL-Destination-Connector zum Laden benötigt.
- **Sync-Modus Cursor (`updatedat`)** statt CDC: vermeidet zusätzliche WAL-/Replication-Konfiguration (siehe offene Frage im Zwischenbericht).
