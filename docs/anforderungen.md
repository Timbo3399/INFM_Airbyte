...
| **4 Mapping Studenten/Personal** | Random-Daten, Account-Generator, in neue Tabellen schreiben | über dbt oder Custom-Code | **entsperrt.** Der frühere Blocker (defekte `hso_students.csv`) ist gelöst, `scripts/mapping/generate_accounts.py` liegt vor | Generator gegen die geladenen 5.052 Zeilen laufen lassen und Ergebnis dokumentieren |
| **5 IdM-System** | `hso_personal` und `hso_students` nach `hso_user` (MySQL), Sync bei Änderung, Bild-Verknüpfung | gut, Incremental mit Dedup | **Vorarbeit erledigt.** Die Incremental-Strategien sind gemessen und verglichen (siehe [call-notes-2026-06-16.md](call-notes-2026-06-16.md)) | Cursor `updatedat` und Primärschlüssel setzen, Connection für `hso_user` anlegen |
